...
class Canvas(app.Canvas):
    ...
    def __init__(self, world):
        ...
        #                     ('a_fg_color', np.float32, 4),
        #                     ('a_size', np.float32, 1)])
        # plot['a_position'] = 0.45 * np.random.randn(n, 3)
        self.plot['a_bg_color'] = np.random.uniform(0.85, 1.00, (n, 4))
        self.plot['a_fg_color'] = 0, 0, 0, 1
        self.plot['a_size'] = 9
        # Create vertices
        #np.random.uniform(5*ps, 10*ps, n)
        u_linewidth = 1.0
        u_antialias = 1.0
